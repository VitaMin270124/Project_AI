...
def metrics_np(y_true, y_pred, metric_name, metric_type='standard', drop_last = False, mean_per_class=False, verbose=False):
    """ 
    ...
    if not flag_soft:
        if num_classes>1:
            ...
            y_pred = np.array([ np.argmax(y_pred, axis=-1)==i for i in range(num_classes) ]).transpose(1,2,3,0) # B*W*H*N -> B*W*H*N
...
def convert_semantic_mask(mask, nc):
    if len(mask.shape) == 2 or (len(mask.shape) == 3 and mask.shape[-1] == 1):
        print('Converting semantic mask to one-hot encoding...')
        if nc == 3:
            mask = np.stack([mask==0, mask==1, mask==2], axis=-1).astype(np.float32)
        if nc == 8:
            mask = np.stack([mask==0, mask==1, mask==2, mask==3, mask==4, mask==5, mask==6, mask==7], axis=-1).astype(np.float32)
